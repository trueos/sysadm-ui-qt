...
class freenas_client : public QObject{
	Q_OBJECT
public:
...
private:
	QWebSocket *SOCKET;
	/*QString chost, cauthkey*/
        QString cuser, cpass; //current host/authkey/user/pass
...
signals:
	...
	void newReply(QJsonObject data);
	//void bridgeReply(QString bridgeID, QString ID, QString namesp, QString name, QJsonValue args);
	//void NewEvent(freenas_client::EVENT_TYPE, QJsonValue);
	//void bridgeEvent(QString bridgeID, freenas_client::EVENT_TYPE, QJsonValue);
